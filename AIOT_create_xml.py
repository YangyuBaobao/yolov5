...
def Create_head(img_name,width,height):
    with open(img_name+'.xml',"w",encoding='UTF-8') as f:
        f.write('<annotation>\n')
...
def Add_object(img_name,label,x1,y1,x2,y2):
    with open(img_name+'.xml',"a",encoding='UTF-8') as f:
        f.write('       <object>\n')
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            Create_head(p.name[:-4],im0.shape[1],im0.shape[0])#########################################################################################################################
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        Add_object(p.name[:-4],label[:-5],c1[0],c1[1],c2[0],c2[1])
            Add_end(p.name[:-4])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='lss.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default=r'data\images', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--img-size', type=int, default=640, help='inference size (pixels)')
    parser.add_argument('--conf-thres', type=float, default=0.28, help='object confidence threshold')
